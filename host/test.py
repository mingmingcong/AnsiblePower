...
Options = namedtuple('Options',
                     ['connection', 'module_path', 'forks', 'remote_user', 'private_key_file', 'ssh_common_args',
                      'ssh_extra_args', 'sftp_extra_args', 'scp_extra_args', 'become', 'become_method', 'become_user',
                      'verbosity', 'check'])
...
options = Options(connection='local', module_path='/path/to/mymodules', forks=100, remote_user=None,
                  ...
                  scp_extra_args=None, become=None, become_method=None, become_user=None, verbosity=None, check=False)
...
play_source = dict(
    ...
    hosts='localhost',
    gather_facts='no',
    tasks=[dict(action=dict(module='shell', args='hostname'),name='ddwdwdwdw')]
)
